...
11. only fresh GitHub evidence may project `REVIEW -> DONE` authoritatively. A PR is an effective linked requirement only when GitHub PullRequest.closingIssuesReferences contains the source Issue; a bare `cross-referenced` timeline event or PR/body mention is not relationship evidence. Normally every effective linked required PR must be merged into the target branch. A historical `closed + unmerged` PR may be excluded only when the source Issue is closed, no effective linked PR remains open, and a newer effective linked PR with the exact same non-empty head ref is merged into the target branch. Different-head, ambiguous, or still-open lineages remain `review`.
