...
Main reads the source Issue/root task to establish identity and authorization, but it should not make every Developer reconstruct the entire Issue/PR discussion. The Investigator handoff is the default implementation context, with exact source references retained for bounded verification. Developer verification is limited to bounded falsification against fresh implementation evidence: if the selected handoff survives, Developer proceeds to RED -> implementation -> GREEN; if material evidence contradicts it, Developer stops and returns that contradiction for a fresh Investigator pass rather than constructing a replacement root-cause model or reopening broad investigation.
