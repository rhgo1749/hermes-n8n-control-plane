...
- role restrictions on lifecycle reconciliation mean external/downstream/peer lifecycle state, not the worker's own mandatory terminal handoff. A loaded skill rule that forbids Kanban mutation for a `delegate_task` child is narrower still: it applies only to an actual delegated child, not to a dispatcher-owned specialist whose assignment happens to be read-only. Every specialist must terminate its own assigned run through the injected Hermes worker protocol (`kanban_complete`, `kanban_request_review`, `kanban_request_changes`, or `kanban_block` as appropriate to that run);
...
`max_expansions=1`, dispatcher-enforced `max_runtime_seconds=7200`,
...
tasks use exactly 7200 seconds, and an explicitly marked `runtime_class=large` implementation
may use exactly 10800 seconds. The repository-owned structured pre-tool wrapper canonicalizes these runtime values before durable materialization, so the model does not choose the numeric standard runtime. Terminal create bypasses cannot be rewritten and therefore fail closed unless they carry the exact canonical runtime. Every newly created H4V3 execution task carries
