...
The exact sanitized local command `automation/hermes/scripts/deploy-intake-edge.sh --hermes-home "$HOME/.hermes" --dry-run` was attempted from the final candidate checkout. Result: `NOT PASS` with exit 2 before candidate validation because this worker profile's `$HOME/.hermes` does not contain the active runtime `scripts` directory/configuration; no live runtime files were changed. A repository-equivalent rerun with a guarded operator-supplied `HERMES_HOME` pointing to a valid isolated runtime layout returned `PASS` (`dry-run: candidate validated` and candidate cleanup). The live mutation/canary was not run because this worker is not authorized to merge, access the private host services, or mutate the active runtime/worktree/Kanban DB; therefore the automation stop state remains `HOST_VALIDATION_REQUIRED`.
