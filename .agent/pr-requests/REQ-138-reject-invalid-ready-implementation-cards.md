...
## 0.2 Follow-up hotfix — shell-hook descendant fence false positive

Fresh live verification after the profile-local hook deployment exposed a second boundary bug on rework run #691. The dispatcher-owned `kanban-main` worker was clean at spawn, but Hermes intentionally marks *subprocesses launched by a Kanban worker* with `HERMES_DELEGATED_CHILD_CONTEXT=1`. The stable shell hook is one such subprocess, and its nested workspace-binding process calls canonical `create_task()` for atomic materialize/readback verification. Carrying the subprocess-only fence into that nested process made every legitimate native `kanban_create` fail as if Main itself were a `delegate_task` child.

The bounded fix clears that descendant fence only when the hook payload is the native structured `kanban_create` surface. The `terminal` path keeps the fence, so CLI/ad-hoc create remains denied. This is safe with the current Hermes authorization order: tool scope is evaluated before `pre_tool_call`, and real `delegate_task` children do not expose `kanban_*` tools. Regression coverage pins both halves of the boundary, while Hermes' own delegate-isolation tests independently prove the native Kanban toolset is absent and Kanban CLI mutation is denied in delegated children.

## 0.1 Follow-up hotfix — profile-local deployment drift

A fresh 2026-09-13 live read-back after later runtime work found that the earlier deployment evidence covered only `/home/hermes/.hermes/config.yaml`. The global config was correct (`kanban-workspace-guard.py` references `0`, stable lifecycle wrapper matchers `3`), but every H4V3 profile-local config still had two legacy workspace-hook entries and zero stable wrapper entries: `kanban-main`, `kanban-investigator`, `kanban-developer`, `kanban-reviewer`, and `kanban-designer`. In particular, `kanban-main/config.yaml` still routed structured `kanban_create` and `terminal` through the superseded 2026-08-26 `kanban-workspace-guard.py`, so a valid structured create could continue to fail even though the global config and new guard file were present.

The repository-owned deployer must therefore treat the global config plus all five H4V3 profile configs as one preflighted deployment surface. Before any mutation it requires all six configs to exist, renders and validates the same approved stable lifecycle hook into six candidate configs, and on apply creates timestamped backups before switching those candidates. Dry-run must leave all six source configs byte-identical. Regression coverage exercises both dry-run and an isolated real apply and asserts that all six configs converge to exactly the three stable matchers (`kanban_block`, `kanban_create`, `terminal`, all fail-closed) with no legacy workspace-hook reference. The superseded runtime file itself remains untouched for rollback archaeology.

The latest fetched integration base for this follow-up is `origin/main=e8ee13718f8eb5b067f0d8743b1a73ecd43517f8`. PR #149 remains the only delivery surface; no replacement PR is permitted, and merge/auto-merge remains human/user authority.
