...
_OLD_LEAD_CONTRACT = """## Luna lead execution contract
...
2. Read the repository's `AGENTS.md`, applicable router (`AGENTS_PROJECT.md` / `Docs/AGENTS.md` where present), canonical docs, and every repository contract path listed in Provenance from the current `origin/{config.default_branch}`.
3. Inspect only enough current `origin/{config.default_branch}`, relevant source/tests, and overlapping PR state to recover scope and route safely. Do not modify the shared checkout directly.
...
def _install_completion_contract_overlay(module: ModuleType) -> None:
    ...
    def patched_task_body(*args: Any, **kwargs: Any) -> str:
        ...
        if _OLD_LEAD_CONTRACT not in rendered:
            raise RuntimeError(
