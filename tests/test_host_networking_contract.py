...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="n8n-host-networking-") as raw:
        ...
        config_home = Path(raw) / "config-home"
        user_home = Path(raw) / "home"
        expected_state_root = state_home / "hermes-n8n-control-plane"
        expected_env_file = config_home / "hermes-n8n-control-plane" / "n8n.env"
        ...
        assert not expected_env_file.exists()
        ...
        values = read_fixture_env(expected_env_file)
        ...
        assert values["HERMES_N8N_ENV_FILE"] == str(expected_env_file)
        assert values["HERMES_N8N_STATE_ROOT"] == str(expected_state_root)
        assert expected_env_file.stat().st_mode & 0o777 == 0o600
        ...
        with expected_env_file.open("a", encoding="utf-8") as stream:
        ...
        migrated_values = read_fixture_env(expected_env_file)
        assert "LEASE_HERMES_BASE_URL" not in migrated_values
        assert migrated_values["HERMES_N8N_ENV_FILE"] == str(expected_env_file)
        assert migrated_values["HERMES_N8N_STATE_ROOT"] == str(expected_state_root)

        calls.unlink()
        expected_env_file.write_text(
            "N8N_PORT=9119\n"
    ...
    print(
        ...
        '"external_runtime_config": true, "external_state_root": true, '
        '"arbitrary_port_rejected": true, "invalid_runtime_port_rejected_before_docker": true}'
    )
    return 0
